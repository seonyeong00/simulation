...
def prep_ml_data_split(deg_data_file,
    ...
    test = pd.read_csv(test_genes,sep="\t",header=None,compression='infer').values[:,0]
    validate = pd.read_csv(validate_genes,sep="\t",header=None,compression='infer').values[:,0]
    train = pd.read_csv(train_genes,sep="\t",header=None,compression='infer').values[:,0]
